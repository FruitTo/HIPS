...
HWLOC_top_srcdir="/home/fruitto/Project/engine/bin/dependencies/hwloc-2.12.1"
HWLOC_top_builddir="/home/fruitto/Project/engine/bin/dependencies/hwloc-2.12.1"
...
(
  ...
  echo
  echo "# List of default NUMA Nodes in a Package"
  $calc --if xml --input $xmldir/64intel64-fakeKNL-SNC4-hybrid.xml --default-nodes -I NUMANode -p Package:0
  echo
  echo "# Number of Memory Tiers"
  $calc --if xml --input $xmldir/64intel64-fakeKNL-SNC4-hybrid.xml -N memorytier all
  echo
  echo "# Memory Tier of a single node"
  $calc --if xml --input $xmldir/64intel64-fakeKNL-SNC4-hybrid.xml -I memorytier node:4
  echo
  ...
  echo
  echo "# Number of CPU kinds"
  $calc --if xml --input $linuxdir/fakeheterocpunuma.output -N cpukind 0xf0f0
  echo
  echo "# List of CPU kinds"
  $calc --if xml --input $linuxdir/fakeheterocpunuma.output -I cpukind 0xff00
  echo
