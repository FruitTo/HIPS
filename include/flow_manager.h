...
// Flow key generation
std::string createFlowKey(PacketInfo *packet);

// Bloom filter functions
size_t hash1(const std::string &key);
size_t hash2(const std::string &key);
size_t hash3(const std::string &key);
bool bloomContains(const std::string &flow_key, uint16_t bloom_counters[]);
void bloomAdd(const std::string &flow_key, uint16_t bloom_counters[]);
...
void createNewFlow(PacketInfo *packet, const std::string &flow_key,
                   const std::string &protocol, const NetworkConfig *conf,
                   std::unordered_map<std::string, FlowEntry> &flow_table,
                   uint16_t bloom_counters[]);
void cleanupExpiredFlows(std::unordered_map<std::string, FlowEntry> &flow_table);
...
inline bool bloomContains(const std::string &flow_key, uint16_t bloom_counters[]) {
...
inline void setFlowbit(FlowEntry& flow, const std::string& bit_name, bool value) {
...
inline void cleanupExpiredFlows(std::unordered_map<std::string, FlowEntry> &flow_table) {
...
inline void createNewFlow(PacketInfo *packet, const std::string &flow_key,
  ...
  if (protocol == "udp" && packet->udp) {
    ...
    if (packet->src_addr == *conf->HOME_NET) {
      new_flow.server_port = static_cast<uint16_t>(std::stoi(packet->udp->sport));
      new_flow.client_port = static_cast<uint16_t>(std::stoi(packet->udp->dport));
    } else {
      new_flow.client_port = static_cast<uint16_t>(std::stoi(packet->udp->sport));
      new_flow.server_port = static_cast<uint16_t>(std::stoi(packet->udp->dport));
    }
    ...
    if (packet->src_addr == *conf->HOME_NET) {
      new_flow.server_port = static_cast<uint16_t>(std::stoi(packet->tcp->sport));
      new_flow.client_port = static_cast<uint16_t>(std::stoi(packet->tcp->dport));
    } else {
      new_flow.client_port = static_cast<uint16_t>(std::stoi(packet->tcp->sport));
      new_flow.server_port = static_cast<uint16_t>(std::stoi(packet->tcp->dport));
    }
