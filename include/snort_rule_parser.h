...
private:
    ...
    static std::string unescapeContent(const std::string& content) {
        ...
        while (std::regex_search(result, match, hex_pattern)) {
            ...
            result = std::regex_replace(result, hex_pattern, std::string(1, hex_char), std::regex_constants::format_first_only);
        }
    ...
    static ClassType parseClassType(const std::string& classtype_str) {
        static std::unordered_map<std::string, ClassType> classtype_map = {
            ...
            {"unusual-client-port-connection", ClassType::UNUSUAL_CLIENT_PORT_CONNECTION},
    ...
    static Operator parseOperator(const std::string& op_str) {
        ...
        if (op_str == "<>") return Operator::RANGE_EXCLUSIVE;
        return Operator::EQUAL;
    }
    
    static ByteTestOperator parseByteTestOperator(const std::string& op_str) {
        if (op_str == "<") return ByteTestOperator::LESS_THAN;
        if (op_str == ">") return ByteTestOperator::GREATER_THAN;
        if (op_str == "<=") return ByteTestOperator::LESS_EQUAL;
        if (op_str == ">=") return ByteTestOperator::GREATER_EQUAL;
        if (op_str == "=") return ByteTestOperator::EQUAL;
        if (op_str == "&") return ByteTestOperator::BITWISE_AND;
        if (op_str == "^") return ByteTestOperator::BITWISE_XOR;
    ...
    static Endianness parseEndianness(const std::string& endian_str) {
        if (endian_str == "big") return Endianness::BIG;
        if (endian_str == "little") return Endianness::LITTLE;
        return Endianness::BIG; // default
    }

    static Width parseWidth(const std::string& width_str) {
        if (width_str == "8") return Width::BITS_8;
        if (width_str == "16") return Width::BITS_16;
        if (width_str == "32") return Width::BITS_32;
        return Width::BITS_8; // default
    }

public:
    static std::vector<Rule> parseRulesFromFile(const std::string& filename) {
        ...
        while (std::getline(file, line)) {
            ...
            if (line.back() != '\\' && current_rule.find('(') != std::string::npos && 
                ...
                try {
                    ...
                    std::cerr << "Warning: Failed to parse rule: " << current_rule << std::endl;
    ...
    static Rule parseRule(const std::string& rule_text) {
        ...
        // Pattern 1: Full format: action protocol src_addr src_port direction dst_addr dst_port
        std::regex header_regex1(R"(^(\w+)\s+(\w+)\s+([^\s]+)\s+([^\s]+)\s+(->|<>|<-)\s+([^\s]+)\s+([^\s]+)\s*\(\s*(.*?)\s*\)$)");
        ...
        if (std::regex_match(rule_text, header_match, header_regex1)) {
            ...
            throw std::runtime_error("Invalid rule format: " + rule_text.substr(0, 100));
        }
...
private:
    ...
    static void parseNetworkParameters(const std::string& params, Rule& rule) {
        ...
        else if (parts.size() == 3 && (parts[0] == "->" || parts[0] == "<>" || parts[0] == "<-")) {
    ...
    static void parseOptions(const std::string& options_text, RuleOption& rule_option) {
    ...
    static void parseIndividualOption(const std::string& option, RuleOption& rule_option) {
        ...
        std::string option_value = option_match.size() > 2 ? trim(option_match[2].str()) : "";
        ...
        if (option_name == "msg") {
            ...
            if (option_value.length() >= 2 && option_value[0] == '"' && option_value.back() == '"') {
            ...
            if (!option_value.empty()) {
                rule_option.general.gid = static_cast<uint16_t>(std::stoi(option_value));
            }
        }
        else if (option_name == "sid") {
            if (!option_value.empty()) {
                rule_option.general.sid = static_cast<uint16_t>(std::stoi(option_value));
            }
        }
        ...
        else if (option_name == "offset" && !rule_option.payload.content.empty()) {
            if (!option_value.empty()) {
                rule_option.payload.content.back().offset = std::stoi(option_value);
            }
        }
        else if (option_name == "distance" && !rule_option.payload.content.empty()) {
            if (!option_value.empty()) {
                rule_option.payload.content.back().distance = std::stoi(option_value);
            }
        }
        else if (option_name == "within" && !rule_option.payload.content.empty()) {
            if (!option_value.empty()) {
                rule_option.payload.content.back().within = std::stoi(option_value);
            }
        }
        else if (option_name == "nocase" && !rule_option.payload.content.empty()) {
            ...
            if (!rule_option.payload.content.empty()) {
                if (option_value.empty()) {
                    ...
                    if (std::regex_match(option_value, fp_match, fp_regex)) {
                        rule_option.payload.content.back().fast_pattern = true;
                        rule_option.payload.content.back().fast_pattern_offset = std::stoi(fp_match[1].str());
                        rule_option.payload.content.back().fast_pattern_length = std::stoi(fp_match[2].str());
                    }
                }
            }
        }
        else if (option_name == "fast_pattern_only" && !rule_option.payload.content.empty()) {
            rule_option.payload.content.back().fast_pattern_only = true;
        }
        // Parse other payload options
            ...
            if (!option_value.empty()) {
                BerData ber_data;
                ber_data.type = static_cast<uint8_t>(std::stoi(option_value, nullptr, 0)); // Support hex
        ...
        // Skip unknown options
            ...
            // Uncomment for debugging: std::cout << "Skipping unknown option: " << option_name << std::endl;
        }
    }
    
    static void parseContentWithModifiers(const std::string& value, PayloadOption& payload) {
        Content content;
    ...
    static void parseContentOption(const std::string& value, PayloadOption& payload) {
        Content content;
    ...
    static void parsePCREOption(const std::string& value, PayloadOption& payload) {
        PCRE pcre;
    ...
    static void parseDsizeOption(const std::string& value, PayloadOption& payload) {
        Dsize dsize;
    ...
    static void parseIsDataAtOption(const std::string& value, PayloadOption& payload) {
        IsDataAt isdataat;
    ...
    static void parseBufferLenOption(const std::string& value, PayloadOption& payload) {
        BufferLen bufferlen;
    ...
    static void parseByteExtractOption(const std::string& value, PayloadOption& payload) {
            ...
            for (size_t i = 3; i < parts.size(); i++) {
                ...
                if (modifier == "relative") {
                    ...
                    if (std::regex_match(modifier, mult_match, mult_regex)) {
                        byte_extract.multiplier = static_cast<uint16_t>(std::stoi(mult_match[1].str()));
                    }
                    ...
                    if (std::regex_match(modifier, align_match, align_regex)) {
                        byte_extract.align = static_cast<uint8_t>(std::stoi(align_match[1].str()));
                    }
    ...
    static void parseByteTestOption(const std::string& value, PayloadOption& payload) {
        ByteTest byte_test;
    ...
    static void parseByteJumpOption(const std::string& value, PayloadOption& payload) {
        ByteJump byte_jump;
        ...
        if (parts.size() >= 2) {
            ...
            for (size_t i = 2; i < parts.size(); i++) {
                ...
                if (modifier == "relative") {
                    ...
                    std::smatch mult_match;
                    if (std::regex_match(modifier, mult_match, mult_regex)) {
                        byte_jump.multiplier = static_cast<uint16_t>(std::stoi(mult_match[1].str()));
                    }
                } else if (modifier.find("base") == 0) {
                    std::regex base_regex(R"(^base\s+(\w+)$)");
                    std::smatch base_match;
    ...
    static void parseBase64DecodeOption(const std::string& value, PayloadOption& payload) {
    ...
    static void parseFlowBitsOption(const std::string& value, NonePayloadOption& nonpayload) {
        ...
        std::regex flowbit_regex(R"(^(set|unset|isset|isnotset|noalert)(?:,(.+))?$)");
        std::smatch flowbit_match;
        
        if (std::regex_match(value, flowbit_match, flowbit_regex)) {
            std::string op = flowbit_match[1].str();
            
            if (op == "set") flowbit.operation = FlowBit::Operation::SET;
            else if (op == "unset") flowbit.operation = FlowBit::Operation::UNSET;
            else if (op == "isset") flowbit.operation = FlowBit::Operation::ISSET;
            else if (op == "isnotset") flowbit.operation = FlowBit::Operation::ISNOTSET;
            else if (op == "noalert") flowbit.operation = FlowBit::Operation::NOALERT;
    ...
    static void parseFlagsOption(const std::string& value, NonePayloadOption& nonpayload) {
        TCPFlags flags;
        ...
        if (!flag_str.empty() && (flag_str[0] == '+' || flag_str[0] == '*' || flag_str[0] == '!')) {
        ...
        for (char c : flag_str) {
            switch (c) {
                case 'F': flags.fin = true; break;
                case 'S': flags.syn = true; break;
                case 'R': flags.rst = true; break;
                case 'P': flags.psh = true; break;
                case 'A': flags.ack = true; break;
                case 'U': flags.urg = true; break;
                case 'E': flags.ece = true; break;
                case 'C': flags.cwr = true; break;
            }
        }
    ...
    static void parseIPProtoOption(const std::string& value, NonePayloadOption& nonpayload) {
    ...
    static void parseTTLOption(const std::string& value, NonePayloadOption& nonpayload) {
        TTL ttl;
    ...
    static void parseIDOption(const std::string& value, NonePayloadOption& nonpayload) {
        ID id;
    ...
    static void parseFragbitsOption(const std::string& value, NonePayloadOption& nonpayload) {
        ...
        if (!bits_str.empty() && (bits_str[0] == '+' || bits_str[0] == '*' || bits_str[0] == '!')) {
        ...
        for (char c : bits_str) {
            switch (c) {
                case 'M': fragbits.more_fragments = true; break;
                case 'D': fragbits.dont_fragment = true; break;
                case 'R': fragbits.reserved = true; break;
            }
        }
    ...
    static void parseITypeOption(const std::string& value, NonePayloadOption& nonpayload) {
        IType itype;
    ...
    static void parseICodeOption(const std::string& value, NonePayloadOption& nonpayload) {
        ICode icode;
    ...
    static void parseICMP_IDOption(const std::string& value, NonePayloadOption& nonpayload) {
        ICMP_ID icmp_id;
    ...
    static void parseICMP_SEQOption(const std::string& value, NonePayloadOption& nonpayload) {
        ICMP_SEQ icmp_seq;
    ...
    static void parseSEQOption(const std::string& value, NonePayloadOption& nonpayload) {
        SEQ seq;
    ...
    static void parseACKOption(const std::string& value, NonePayloadOption& nonpayload) {
        ACK ack;
...
void printRule(const Rule& rule) {
    ...
    std::cout << "Source: " << rule.src_addr.value_or("N/A") << ":" << rule.src_port.value_or("N/A") << std::endl;
    std::cout << "Direction: " << rule.direction.value_or("N/A") << std::endl;
    std::cout << "Destination: " << rule.dst_addr.value_or("N/A") << ":" << rule.dst_port.value_or("N/A") << std::endl;
    ...
    std::cout << "ClassType: " << static_cast<int>(rule.option.general.classtype) << std::endl;
    ...
    if (!rule.option.payload.content.empty()) {
        std::cout << "Content patterns: " << rule.option.payload.content.size() << std::endl;
    ...
    if (rule.option.payload.pcre && !rule.option.payload.pcre->empty()) {
        std::cout << "PCRE patterns: " << rule.option.payload.pcre->size() << std::endl;
        for (size_t i = 0; i < rule.option.payload.pcre->size(); i++) {
            const auto& pcre = (*rule.option.payload.pcre)[i];
            std::cout << "  [" << i << "] " << pcre.pattern;
            if (pcre.negate) std::cout << " (negated)";
            if (pcre.i) std::cout << " (i)";
            if (pcre.s) std::cout << " (s)";
            if (pcre.m) std::cout << " (m)";
            std::cout << std::endl;
        }
    }
    ...
    if (rule.option.payload.dsize) {
        ...
        switch (rule.option.payload.dsize->op) {
            case Operator::GREATER_THAN: std::cout << ">"; break;
            case Operator::LESS_THAN: std::cout << "<"; break;
            case Operator::EQUAL: std::cout << "="; break;
            case Operator::NOT_EQUAL: std::cout << "!="; break;
            case Operator::GREATER_EQUAL: std::cout << ">="; break;
            case Operator::LESS_EQUAL: std::cout << "<="; break;
            case Operator::RANGE_EXCLUSIVE: std::cout << "<>"; break;
            case Operator::RANGE_INCLUSIVE: std::cout << "-"; break;
        }
    ...
    if (rule.option.payload.byte_tests && !rule.option.payload.byte_tests->empty()) {
        std::cout << "Byte tests: " << rule.option.payload.byte_tests->size() << std::endl;
    }
    
    if (rule.option.payload.ber_data_list && !rule.option.payload.ber_data_list->empty()) {
        std::cout << "BER data checks: " << rule.option.payload.ber_data_list->size() << std::endl;
    }
    ...
    if (rule.option.nonpayload.ttl) {
        std::cout << "TTL check: " << static_cast<int>(rule.option.nonpayload.ttl->min_value) << std::endl;
    }
...
void demonstrateParser(const std::string& rules_file = "./rule/snort3-community.rules") {
    try {
        std::cout << "Attempting to parse rules from: " << rules_file << std::endl;
        std::vector<Rule> rules = SnortRuleParser::parseRulesFromFile(rules_file);
        
        std::cout << "Successfully parsed " << rules.size() << " rules from " << rules_file << std::endl;
        
        if (rules.empty()) {
            std::cout << "No rules were parsed. Check if the file exists and has valid rules." << std::endl;
            return;
        }
        ...
        int count = 0;
        for (const auto& rule : rules) {
            if (count >= 3) break; // Show only first 3 rules
            printRule(rule);
            count++;
        }
