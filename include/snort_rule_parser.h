...
public:
    static std::map<std::string, std::vector<Rule>> parseRulesFromFile(const std::string &filename)
    {
...
void demonstrateParser(const std::string& rules_file = "./rule/snort3-community.rules") {
    try {
        ...
        std::cout << "Successfully parsed " << total_rules << " rules from " << rules_file << std::endl;
