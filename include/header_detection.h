...
void testRule() {
    std::cout << "=== Testing Header Detection Engine ===" << std::endl;
    
    // Test Case 1: Basic TCP rule with HOME_NET
    {
        std::cout << "\n--- Test 1: HOME_NET -> Any (TCP:80) ---" << std::endl;
        
        PacketInfo packet;
        packet.protocol = "tcp";
        packet.src_addr = "192.168.1.100";
        packet.dst_addr = "8.8.8.8";
        packet.tcp.emplace();
        packet.tcp->sport = "12345";
        packet.tcp->dport = "80";
        
        NetworkConfig conf;
        conf.HOME_NET = "192.168.1.100";
        conf.EXTERNAL_NET = "!192.168.1.100";
        conf.HTTP_SERVERS = true;
        
        Rule rule;
        rule.protocol = "tcp";
        rule.src_addr = "$HOME_NET";
        rule.dst_addr = "any";
        rule.direction = "->";
        rule.src_port = "any";
        rule.dst_port = "80";
        rule.option.general.service = std::vector<std::string>{"http"};
        
        bool result = headerDetection(rule, packet, conf);
        std::cout << "Result: " << (result ? "MATCH" : "NO MATCH") << std::endl;
        std::cout << "Packet: " << packet.src_addr << ":" << packet.tcp->sport 
                  << " -> " << packet.dst_addr << ":" << packet.tcp->dport << std::endl;
    }
    
    // Test Case 2: External -> HOME_NET
    {
        std::cout << "\n--- Test 2: External -> HOME_NET (TCP:22) ---" << std::endl;
        
        PacketInfo packet;
        packet.protocol = "tcp";
        packet.src_addr = "8.8.8.8";
        ...
        Rule rule;
        rule.protocol = "tcp";
        rule.src_addr = "$EXTERNAL_NET";
        rule.dst_addr = "$HOME_NET";
        rule.direction = "->";
        rule.src_port = "any";
        rule.dst_port = "22";
    ...
    // Test Case 3: Service disabled
    {
        std::cout << "\n--- Test 3: Service Disabled ---" << std::endl;
        ...
        NetworkConfig conf;
        conf.HOME_NET = "192.168.1.100";
        conf.HTTP_SERVERS = false;  // Disabled!
        ...
        bool result = headerDetection(rule, packet, conf);
        std::cout << "Result: " << (result ? "MATCH" : "NO MATCH") << " (Expected: NO MATCH)" << std::endl;
    }
    
    // Test Case 4: IP rule with ip_proto
    {
        std::cout << "\n--- Test 4: IP Rule (ICMP) ---" << std::endl;
        
        PacketInfo packet;
        packet.protocol = "icmp";
        packet.src_addr = "8.8.8.8";
        packet.dst_addr = "192.168.1.100";
        packet.ip_proto = 1; // ICMP
        packet.icmp.emplace();
        ...
        rule.dst_addr = "$HOME_NET";
        rule.direction = "->";
        rule.option.nonpayload.ip_proto.emplace();
        rule.option.nonpayload.ip_proto->op = Operator::EQUAL;
        rule.option.nonpayload.ip_proto->protocol = static_cast<uint8_t>(1);
        
        bool result = headerDetection(rule, packet, conf);
        std::cout << "Result: " << (result ? "MATCH" : "NO MATCH") << std::endl;
        std::cout << "Packet: ICMP " << packet.src_addr << " -> " << packet.dst_addr << std::endl;
    }
    
    std::cout << "\n=== Header Detection Tests Complete ===" << std::endl;
}
